...
def run_epoch(data_iter,model,loss_fn,optimizer: Optional[torch.optim.Optimizer],scheduler=None,epoch_num = 0,is_train = True,device = "cpu"):
    if is_train:
        model.train()
    else:
        model.eval()
...
def evaluate_bleu(model,test_dataloader,tgt_vocab_inv,device = "cpu",max_len = 100):
    model.eval()
    targets = []
    outputs = []
...
def run_training_experiment():
    ...
    wandb.init(project="da6401-A3", config={
        ...
        "dropout": 0.1, "warmup_steps": 4000, "batch_size": 32, "epochs": 10
    })
    cfg = wandb.config
    ...
    loss_fn = LabelSmoothingLoss(len(train_data_obj.tgt_vocab), pad_idx=1, smoothing=0.0)

    for epoch in range(cfg.epochs):
        train_loss = 0
        val_loss = 0
        train_loss = run_epoch(train_loader, model, loss_fn, optimizer, scheduler, epoch, True, device,train_data_obj.src_vocab)
        val_loss = run_epoch(val_loader, model, loss_fn, None, None, epoch, False, device,val_data_obj.src_vocab)
        print("done")
        # bleu = evaluate_bleu(model, val_loader, train_data_obj.tgt_inv_vocab, device)
